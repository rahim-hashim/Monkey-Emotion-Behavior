...
# Save path
VIDEO_SAVE_PATH = '/Users/rahimhashim/Desktop'
...
def timestamp_image_formatter(timestamp_image_file):
		'''
		timestamp_image_formatter splits the t0 and t1 .csv files
...
def img_to_vid():
		...
		multiple_trials_flag = input('Multiple Trials (Y | N)')
		...
		# # Old implementation for specifying only 1 session
		# if len(image_folder) > 1:
		#     specified_index = int(input('Which session (input a number i.e. 1, 2,...) would you like to specify: '))
		#     try:
		#         specified_folder = image_folder[specified_index-1]
		#     except:
		#         sys.exit('\nSpecified session outside of range')
		# else:
		#     specified_folder = image_folder[0]
		...
		min_dist_session = closest_timestamp(trial_start_datetime, '', min_dist_timestamp_list_t1_start)
		session_index = 0
		if min_dist_session == min_dist_timestamp_list_t1_start[0]:
		...
		frame_timestamp_start = min_dist_timestamp_list_t1_start[session_index]
		frame_timestamp_end = min_dist_timestamp_list_t1_end[session_index]
		frame_datetimes = timestamp_image_file_t1_list[session_index]
		images_cam0 = images_cam0_list[session_index]
		images_cam1 = images_cam1_list[session_index]
		...
		images_cam_list = [images_cam0, images_cam1]
		for c_index, cam in enumerate(['cam0', 'cam1']):
				print('\n  {}'.format(cam))
				images_cam = images_cam_list[c_index]
				video_name = DATE + '_' + MONKEY + '_trial_' + trial_number_str + '_' + cam + '.mp4'
				...
				# selected_images = sorted_image_order[frame_index_start:frame_index_end] # list index exclusive
				# misordered_image_files = []
				# for i_index, image in enumerate(selected_images):
				#     frame_label_cur = int(image.split('_')[1])
				#     if i_index > 1:
				#         frame_label_last = int(sorted_image_order[i_index-1].split('_')[1])
				#         if frame_label_cur-frame_label_last != 1:
				#             print(image, sorted_image_order[i_index-1])
				#             misordered_image_files.append(image)
