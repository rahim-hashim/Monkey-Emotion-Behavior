...
def raster_by_condition(session_df, behavioral_code_dict, error_dict, session_obj):
	...
	axarr[0][0].text(0, 1.08, 'CS On', ha='center', va='center')
	axarr[0][0].text(cs_time_min, 1.08, 'Delay', ha='center', va='center')
	axarr[0][0].text(trace_time_min, 1.08, 'Outcome', ha='center', va='center')
	axarr[0][0].set_ylabel('Probability of Lick')
	axarr[0][0].set_ylim([0, 1.05])
	axarr[0][0].set_yticks(np.arange(0,1.05,0.1))
	# lick
	...
	axarr[0][0].set_title('Condition {}\n(after {} exposures to each fractal)'.format(condition, TRIAL_THRESHOLD), pad=25)

	axarr[1][0].set_ylabel('Probability of Blink')
	axarr[1][0].set_ylim([0, 1.05])
	axarr[1][0].set_yticks(np.arange(0,1.05,0.1))

	axarr[2][0].set_ylabel('Pupil Diameter')
	axarr[2][0].set_xlabel('Time since visual stimuli onset (ms)')
	...
	for ax_index in range(3):
		...
		axarr[ax_index][1].bar(list(range(num_fractals)), data_probability_mean, color=COLORS, ec='black')
		...
		axarr[ax_index][2].bar(list(range(num_fractals)), data_duration_mean, color=COLORS, ec='black')
		axarr[ax_index][2].set_xticks(list(range(num_fractals)))
		axarr[ax_index][2].set_xticklabels(labels)
		axarr[ax_index][2].set_xlabel('Outcome')
		axarr[ax_index][2].set_ylabel('{}'.format(label_list_dur[ax_index]))
		if ax_index == 2:
