...
iti_time = 1500; % ITI: 1500 ms
set_iti(iti_time);
reward_airpuff_time = 1000;

%% Error Intervals
error_timeout = 1500; % ITI error: 2000 ms
...
if airpuff
    ...
    if airpuff_random_num > 0.5
        ...
        airpuff_struct.side(end+1) = 1; % Left
        disp('Left Airpuff')
    else
        airpuff_stim.Channel = 2;
        airpuff_struct.side(end+1) = 2; % Right
        disp('Right Airpuff')
    end
    airpuff_stim.Waveform = airpuff_waveform;
    airpuff_stim.Frequency = airpuff_freq; % rate of waveform (samples/sec)
    airpuff_stim.WaveformNumber = 1;
    con4.add(airpuff_stim)
else
    airpuff_struct.side(end+1) = 0; % Neither
end
TrialRecord.User.airpuff.side = airpuff_struct.side;
...
function data_print()
    ...
    % eye data
    eye_data = get_analog_data('eye', 500);
